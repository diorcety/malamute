...
store_stream_reader (client_t *self)
{
    ...
    if (stream) {
        ...
        while(lastmsg) {
            zsock_bsend (stream->msgpipe, "ppps", NULL, self, mlm_msg_link (lastmsg), mlm_proto_pattern (self->message));
...
write_message_to_stream (client_t *self)
{
    ...
    mlm_msg_t *lastmsg = (mlm_msg_t *) zlistx_first (stream->lastmsgs);
    while(lastmsg) {
        if (streq (mlm_msg_subject (lastmsg), mlm_msg_subject (msg))) {
            break;
        }
        lastmsg = (mlm_msg_t *) zlistx_next (stream->lastmsgs);
    }
    ...
    zsock_bsend (stream->msgpipe, "ppps", self, NULL, lastmsg, NULL);
}
...
mlm_server_test (bool verbose)
{
...

    {
        ...
        assert (mlm_client_send (producer, "STREAM_TEST", "hello", &msg) >= 0);
